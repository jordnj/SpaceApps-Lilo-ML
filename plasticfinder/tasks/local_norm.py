...
class LocalNormalization(EOTask):
    ...
    @staticmethod
    def normalize(data, mask, method,axis=[1,4], window_size=2):
    ...
    def execute(self,eopatch, method, window_size):
        ...
        normed_bands, m_bands = self.normalize(eopatch.data['BANDS-S2-L1C'], valid_mask, method=method, window_size=window_size)
        
        eopatch.add_feature(FeatureType.DATA, 'NORM_FDI', normed_fdi)
        eopatch.add_feature(FeatureType.DATA, 'NORM_NDVI', normed_ndvi)
        eopatch.add_feature(FeatureType.DATA, 'NORM_BANDS', normed_bands)
