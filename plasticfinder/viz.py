...
def plot_classifications(patchDir, features=None):
    ...
    classifications = patch.data['CLASSIFICATION'][0,:,:,0]     
    ...
    fclassifications = classifications.flatten()
    fclassifications[ (ffdi < 0.007)]  = 0
    
    p_grid = np.array([cols_rgb[val] for val in fclassifications])

    axs[0].set_title("Labels")
    axs[0].imshow(p_grid.reshape(classifications.shape[0], classifications.shape[1],3))
